 ...
 */
    ...
    gVEmailAttachmentName VARCHAR2(30); 
...
BEGIN  
    ...
    -- IMPORTANT: change the second parameter of this procedure call to the email address
    -- to which you want to receive the forecast email. ALSO do this for the procedure call below
    -- in the exception segment.
    SEND_EMAIL('Forecast Data.', '12580092@student.uts.edu.au', 'procedure@uts.edu.au'); -- send email with file.
    ...
    WHEN OTHERS THEN
        ...
        HANDLE_END_OF_FILE();
        SEND_EMAIL('Forecast Data.', '12580092@student.uts.edu.au', 'procedure@uts.edu.au'); -- Change second param here too!
END RM16_FORECAST;
...
PROCEDURE INSERT_INTO_LOCAL (pDateLookingAt IN DATE) IS 
    ...
    vIsHoliday NUMBER;
BEGIN
    vIsHoliday := IS_HOLIDAY(pDateLookingAt);
...
/**
 *  This procedure sends an email from the param 'pSender' to 'pRecipient'
 *  mostly 
 */
PROCEDURE SEND_EMAIL (pSubject IN VARCHAR2,
                     pRecipient IN VARCHAR2,
                     pSender IN VARCHAR2) IS  
                     
    vSMTPConnection UTL_SMTP.CONNECTION;
    vSMTPServerName VARCHAR2(30) := 'postoffice.uts.edu.au';
    vSMTPServerPort NUMBER := 25;     
    vendOfEmail VARCHAR2(100) := '***** This is an automated email, please do not respond. *****'; -- footer.
    vBoundary VARCHAR2(25) := 'aleBoundary'; -- boundary required for SMTP stream.
    vLineDevisor NUMBER  := 12000; -- what we devise by to write lines.
    cnLine VARCHAR2(2) := CHR(13)||CHR(10); -- line break.
BEGIN
    -- open the connection.
    vSMTPConnection := UTL_SMTP.OPEN_CONNECTION(vSMTPServerName, vSMTPServerPort); 
    UTL_SMTP.helo(vSMTPConnection, vSMTPServerName); 
    
    -- prepare credentials.
    UTL_SMTP.mail(vSMTPConnection, pSender); -- sender.
    UTL_SMTP.rcpt(vSMTPConnection, pRecipient); -- recipient.
  
    -- open message.
    UTL_SMTP.open_data(vSMTPConnection);
    
    -- prepare information.
    UTL_SMTP.write_data(vSMTPConnection, 'Date: ' || TO_CHAR(SYSDATE, 'DD-MM-YYYY HH24:MI:SS') || cnLine);  
    UTL_SMTP.write_data(vSMTPConnection, 'To: ' || pRecipient || cnLine);  
    UTL_SMTP.write_data(vSMTPConnection, 'Cc: ');  -- sending only directly.
    UTL_SMTP.write_data(vSMTPConnection, 'From: ' || pSender || cnLine);  
    UTL_SMTP.write_data(vSMTPConnection, 'Subject: ' || 'Forecast Data.' || cnLine);  
    UTL_SMTP.write_data(vSMTPConnection, 'MIME-Version: 1.0' || cnLine);
    UTL_SMTP.write_data(vSMTPConnection, 'Content-Type: multipart/mixed; boundary="' || vBoundary || '"' || cnLine || cnLine);
    
    -- opening email message boundary.
    UTL_SMTP.write_data(vSMTPConnection,cnLine || '--' || vBoundary || cnLine);
    UTL_SMTP.write_data(vSMTPConnection, cnLine || 'Sent From the OMS Database by the PL/SQL application.' || cnLine);
    UTL_SMTP.write_data(vSMTPConnection, cnLine || 'The report data is in the attached file.' || cnLine);
    UTL_SMTP.write_data(vSMTPConnection, cnLine || 'Regards.' || cnLine);
    UTL_SMTP.write_data(vSMTPConnection, cnLine || 'The OMS Database.' || cnLine);
    UTL_SMTP.write_data(vSMTPConnection, cnLine || cnLine || vendOfEmail || cnLine || cnLine);

    -- opening attachment boundary.
    UTL_SMTP.write_data(vSMTPConnection,cnLine || '--' || vBoundary || cnLine);
    UTL_SMTP.write_data(vSMTPConnection, 'Content-Type: ' || 'application/octet-stream' || '; name="' || gVEmailAttachmentName || '"' || cnLine); 
    UTL_SMTP.WRITE_DATA(vSMTPConnection, 'Content-Transfer-Encoding: 7bit'|| cnLine || cnLine);    --7bit

    -- loop text in clob.
    FOR line IN 0 .. TRUNC((DBMS_LOB.getlength(gVDataClob) - 1 )/vLineDevisor) LOOP -- loop for each 'line' in clob
        -- write clob substring into email.
        UTL_SMTP.write_data(vSMTPConnection, DBMS_LOB.substr(gVDataClob, vLineDevisor, line * vLineDevisor + 1));
    END LOOP;
    UTL_SMTP.write_data(vSMTPConnection, cnLine || cnLine);
    
    -- closing boundary.
    UTL_SMTP.write_data(vSMTPConnection, cnLine || '--' || vBoundary|| '--' || cnLine);
    
    UTL_SMTP.close_data(vSMTPConnection);
    UTL_SMTP.quit(vSMTPConnection); 
    COMMON.LOG('** Info: Forecast data sent to ' || pRecipient || ' from ' || pSender);

    EXCEPTION
    WHEN OTHERS THEN
        -- safeguard: anything could go wrong in this process, general exception.
        COMMON.LOG('** Info: Exception in SEND_EMAIL');
        UTL_SMTP.close_data(vSMTPConnection);
END SEND_EMAIL;
